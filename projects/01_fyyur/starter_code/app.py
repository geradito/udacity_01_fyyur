...
def venues():
  ...
  all_venues = Venue.query.with_entities(Venue.id,Venue.name,Venue.city,Venue.state, func.count(Venue.shows).label("num_shows")).join(Show, isouter=True).group_by(Venue.id,Venue.name,Venue.city,Venue.state).all() 
  venues =[]

  for venue in states:
    temp_city = {'city':venue.city, 'state':venue.state, 'venues':[]}
    for x in all_venues:
        if x.city==venue.city and x.state==venue.state:
          temp_venue ={'id':x.id,'name':x.name, 'num_upcoming_shows':x.num_shows  }
          temp_city['venues'].append(temp_venue)
    venues.append(temp_city)
...
def shows():
  ...
  data = []
  for show in shows:
    data.append({"venue_id":show.venue.id,"venue_name":show.venue.name,"artist_id":show.artist.id,"artist_name":show.artist.name,"artist_image_link":show.artist.image_link,"start_time":show.start_time})
